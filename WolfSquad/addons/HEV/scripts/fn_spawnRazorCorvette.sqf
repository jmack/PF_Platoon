...

{
  if (_forEachIndex > 48) then {
    ["Too many units, you will have to wait for the next drop"] remoteExec ["hint", _x];
  } else {
    if (alive _x) then {
      private _unit = _x;
      ...
      if ((_copyCargo isNotEqualTo []) AND { (_copyCargo isNotEqualTo _standardPodCargo)}) then {
      ...
      private _leftOrRightOfShip = ((_forEachIndex mod 2) isNotEqualTo 0);
