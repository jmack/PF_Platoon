/*
  ...
  Description:
    Initializes multiple HEV bays and panels to allow single or multi-bay launches

  Parameters:
    _matrix  // HASHMAP - A hashmap containing the HEV setup matrix.
  
  Returns:
    VOID

  Examples:
    [matrix_here] call WS_fnc_InitHEVs
*/
...
if (!isNil "WS_HEV_Map") exitWith {
  ["WARN", "WS_fnc_InitHEVs has already been called! You cannot initialize twice!"];
};

["INFO", "Initializing HEVs"] call WS_fnc_LogMsg;
...
if (isServer) then {
  ...
  WS_fnc_EnableHEVAlarms = {
    ...
    // handled in the composition. fix this when RC is moved into WS_Core
    _fnc_getElems = missionNamespace getVariable ["WS_fnc_RC_GetElems", { }];
    _fnc_updateElem = missionNamespace getVariable ["WS_fnc_RC_UpdateElem", { }];

    _bay = (WS_HEV_Map get "bays") get _bayName;
    _rcName = _bay get "rcname";
    _dropLights = _bay get "lights";
    _roomLights = [_rcName, "lights"] call _fnc_getElems;

    ["DEBUG", format ["entered WS_fnc_EnableHEVAlarms. bayName: %1, rcName: %2", _bayName, _rcName]] call WS_fnc_LogMsg;

    // turn bay lights off
    [_rcName, "lights", [true]] call _fnc_updateElem;
    ...
    [(_bay get "owner"), _rcName, _dropLights, _roomLights] spawn {
      ...
      if (!(_roomLights#0 getVariable "WS_Light_LastMode")) then {
        _fnc_updateElem = missionNamespace getVariable ["WS_fnc_RC_UpdateElem", { }];
        [_rcName, "lights", [false]] call _fnc_updateElem;
      };
...
// event handlers needed to be added to each client (maybe?)
